...
public class GameTestScenarioBuilder {
    ...
    public void run() {
        /*
         * We work around MockPlayer AABB issues by NOT adding the player to
         * the world for non-vehicle tests.  Instead we:
         ...
         * Player.getDestroySpeed because the mixin modifies the Player class
         * itself, not just ServerPlayer.
         */
     ...
     *
     * <p>On Forge 1.20.1, {@code isEyeInFluid(FluidTags.WATER)} is patched to
     * delegate to {@code isEyeInFluidType(WATER_TYPE)}, which checks the
     * Forge-specific {@code forgeFluidTypeOnEyes} field instead.  We therefore
     * also set that field via reflection when it exists.
     */
    ...
    private void injectFluidOnEyes(Player player) {
        ...
        // 2. Forge path: set the forgeFluidTypeOnEyes field to WATER_TYPE.
        try {
            Field forgeField = Entity.class.getDeclaredField("forgeFluidTypeOnEyes");
            forgeField.setAccessible(true);
            // Resolve ForgeMod.WATER_TYPE via reflection to avoid a hard
            // compile-time dependency on a specific Forge method signature.
            Class<?> forgeMod = Class.forName("net.minecraftforge.common.ForgeMod");
            Field waterTypeHolder = forgeMod.getDeclaredField("WATER_TYPE");
            waterTypeHolder.setAccessible(true);
            // WATER_TYPE is a RegistryObject<FluidType>; call .get() to get the FluidType.
            Object holder = waterTypeHolder.get(null);
            Object waterType = holder.getClass().getMethod("get").invoke(holder);
            forgeField.set(player, waterType);
        } catch (ClassNotFoundException | NoSuchFieldException ignored) {
            // Not running on Forge with fluid type support — vanilla fluidOnEyes is sufficient.
        } catch (ReflectiveOperationException e) {
